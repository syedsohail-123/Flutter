...
// AWS SDK config - using Vercel environment variables
AWS.config.update({
  ...
  try {
    ...
    if (err.code === 'UnrecognizedClientException') {
      res.status(401).json({ 
        ...
        message: "Invalid AWS credentials. Please check your AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY environment variables."
      });
